  ...
  function column(count: number, xOffset: number, fill: string) {
    return Array.from({ length: count }, (_, i) => (
      <circle key={`${xOffset}-${i}`}
        cx={xOffset + (i%COLS)*(D+G)+D/2}
        cy={H - Math.floor(i/COLS)*(D+G)-D/2}
        r={D/2} fill={fill} />
    ));
  }
  const gap=20, totalW=colW*2+gap;
  return (
...
function PersonIcon({ x, y, s, fill, opacity }: { x:number; y:number; s:number; fill:string; opacity:number }) {
  const r = s*0.27;
  return (
    <g opacity={opacity} style={{ transition:"opacity 0.06s" }}>
  ...
  return (
    <div className="flex flex-col gap-1">
      <svg width="180" height="94" viewBox="0 0 180 94">
        ...
        {arc(maxR*.64,Math.min(angleDeg+7,180),0.24)}
        {arc(maxR*.46,angleDeg,0.95)}
...
// ── R: Gradient density bar ── Wilke confidence strip
...
export function VSUPBar({ pct, t }: { pct: number; t: T }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-baseline gap-1">
        <span style={{ fontSize:48, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color:displayColor }}>{Math.round(ap)}</span>
        <span style={{ fontSize:20, fontWeight:200, color:"var(--muted)" }}>%</span>
      </div>
      <div className="relative h-3 rounded-full overflow-hidden" style={{ background:"var(--border)" }}>
        <div style={{ position:"absolute", top:0, left:0, height:"100%", width:`${ap}%`,
          background:displayColor, opacity:0.3+certainty*0.7, transition:"width 0.05s" }} />
      </div>
