  ...
  return (
    <div className="flex flex-col items-start">
      <svg width="180" height="100" viewBox="0 0 180 100">
        ...
        {fillD && <path d={fillD} fill="none" stroke="var(--card-color)" strokeWidth="5" strokeLinecap="round" />}
        <circle cx={ap<=0?cx-r:nx} cy={ap<=0?cy:ny} r="7" fill={ap<=0?"var(--border)":"var(--card-color)"} />
      </svg>
      <div className="flex items-baseline gap-1 -mt-2">
        <span style={{ fontSize:52, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color: "var(--card-color)" }}>{Math.round(ap)}</span>
      ...
      <div className="grid gap-[4px]" style={{ gridTemplateColumns:"repeat(10,1fr)", width:"130px" }}>
        {Array.from({ length:100 }, (_,i) => (
          <div key={i} style={{ width:10, height:10, borderRadius:"50%",
            background: i < filled ? "var(--card-color)" : "var(--border)",
...
export function VerticalBar({ pct, t }: { pct: number; t: T }) {
  ...
  return (
    <div className="flex items-end gap-5">
      <div className="flex flex-col items-center gap-1">
        ...
        <div className="relative" style={{ width:32, height:110, background:"var(--border)" }}>
          ...
          <div style={{ position:"absolute", bottom:0, left:0, right:0,
            height:`${Math.max(pct*p,0)}%`, background:"var(--card-color)",
      ...
      <div>
        <div style={{ fontSize:52, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color: "var(--card-color)" }}>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex" style={{ height:4, gap:2 }}>
        <div style={{ flex:Math.max(ap, 0.01), background:"var(--card-color)", borderRadius:2, transition:"flex 0.05s" }} />
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width="108" height="108" viewBox="0 0 108 108">
        ...
        <circle cx="54" cy="54" r={r} fill="none" stroke="var(--card-color)" strokeWidth="4"
          strokeDasharray={`${fill} ${circ}`} strokeDashoffset={circ/4} strokeLinecap="round" />
        <text x="54" y="48" textAnchor="middle" fontSize="26" fontWeight="200" fill="var(--card-color)" fontFamily="inherit">{Math.round(pct*p)}</text>
      ...
      <div style={{ fontSize:11, lineHeight:2.2 }}>
        ...
        <div style={{ fontSize:20, fontWeight:200, lineHeight:1, color: "var(--card-color)" }}>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="w-full">
        <svg width="100%" height="28" viewBox="0 0 200 28" preserveAspectRatio="xMidYMid meet">
          ...
          <line x1="12" y1="14" x2={thumbX} y2="14" stroke="var(--card-color)" strokeWidth="2" strokeLinecap="round" />
          <circle cx={thumbX} cy="14" r="7" fill="var(--background)" stroke="var(--card-color)" strokeWidth="2" />
        </svg>
...
export function SegmentBlocks({ pct, t }: { pct: number; t: T }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-baseline gap-1">
        <span style={{ fontSize:52, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color: "var(--card-color)" }}>{Math.round(pct*p)}</span>
        <span style={{ fontSize:22, fontWeight:200, color:"var(--muted)" }}>%</span>
      </div>
  ...
  return (
    <div className="flex flex-col gap-2">
      <svg width="180" height="56" viewBox={`0 0 ${W} ${H}`}>
        ...
        <polyline points={points} fill="none" stroke="var(--card-color)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
      <div className="flex items-baseline gap-1">
        <span style={{ fontSize:44, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color: "var(--card-color)" }}>{Math.round(pct*p)}</span>
        <span style={{ fontSize:18, fontWeight:200, color:"var(--muted)" }}>%</span>
        ...
        {Array.from({ length:rows }, (_,row) => {
          ...
          return (
            <div key={row} className="relative h-4 rounded-sm overflow-hidden" style={{ background:"var(--border)" }}>
              <div style={{ width:`${rowPct}%`, height:"100%", background:"var(--card-color)", borderRadius:2, transition:"width 0.05s" }} />
            </div>
          );
        })}
      </div>
...
export function TypoLarge({ pct, t }: { pct: number; t: T }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      <div style={{ fontSize:88, fontWeight:weight, lineHeight:1, letterSpacing:"-0.04em", color: "var(--card-color)" }}>
      ...
      <div className="relative h-px w-full" style={{ background:"var(--border)" }}>
        <div style={{ position:"absolute", top:0, left:0, height:"100%", width:`${pct*p}%`, background:"var(--card-color)", transition:"width 0.05s" }} />
      </div>
