...
/** 10-step color scale: 0% = deep crimson → 100% = forest green */
  ...
  return (
    <div className="flex flex-col items-start">
      <svg width="180" height="100" viewBox="0 0 180 100">
        ...
        <circle cx={pct<=0?cx-r:nx} cy={pct<=0?cy:ny} r="7" fill={pct<=0?"var(--border)":color} />
      </svg>
      <div className="flex items-baseline gap-1 -mt-2">
        <span style={{ fontSize:52, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color }}>{pct}</span>
    ...
    <div className="flex items-end gap-5">
      <div className="flex flex-col items-center gap-1">
        ...
        <div className="relative" style={{ width:32, height:110, background:"var(--border)" }}>
          ...
          <div style={{ position:"absolute", bottom:0, left:0, right:0, height:`${Math.max(pct,2)}%`, background:color }} />
        </div>
        <span style={{ fontSize:10, color:"var(--muted)" }}>0</span>
      </div>
      <div>
        <div style={{ fontSize:52, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color }}>
          {pct}<span style={{ fontSize:22, fontWeight:200, color:"var(--muted)" }}>%</span>
        </div>
  ...
  return (
    <div className="flex items-center gap-6">
      <svg width="108" height="108" viewBox="0 0 108 108">
        ...
        <text x="54" y="48" textAnchor="middle" fontSize="26" fontWeight="200" fill={color} fontFamily="inherit">{pct}</text>
...
// ── G: Segment blocks (10 bars) ───────────────────────────────────────────────
export function SegmentBlocks({ pct, t }: { pct: number; t: T }) {
  const color = probColor(pct);
  const filled = Math.round(pct / 10); // 0–10 filled blocks
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-baseline gap-1">
        <span style={{ fontSize:52, fontWeight:200, lineHeight:1, letterSpacing:"-0.02em", color }}>{pct}</span>
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-col gap-[3px]">
        {Array.from({ length: rows }, (_, row) => {
          const rowPct = Math.max(0, Math.min(100, pct - row * 20)) / 20 * 100;
          return (
            <div key={row} className="relative h-4 rounded-sm overflow-hidden"
              style={{ background:"var(--border)" }}>
              <div style={{ width:`${rowPct}%`, height:"100%", background:color, borderRadius:2 }} />
            </div>
          );
        })}
      </div>
...
export function TypoLarge({ pct, t }: { pct: number; t: T }) {
  const color = probColor(pct);
  // font weight varies with probability: thin at extremes, heavier near 50%
  ...
  return (
    <div className="flex flex-col gap-3">
      <div style={{ fontSize:88, fontWeight:weight, lineHeight:1, letterSpacing:"-0.04em", color }}>
        {pct}<span style={{ fontSize:36, color:"var(--muted)", fontWeight:200 }}>%</span>
      </div>
